...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const { respostas, utm_source, utm_medium, utm_campaign } = body as {
  ...
  if (nome || email || wpp) {
    ...
    // Tenta encontrar lead existente pelo WhatsApp ou e-mail
  ...
  if (response?.id && respostas.length > 0) {
    const answers = respostas
      .filter(r => r.question_id)
      .map(r => ({ response_id: response.id, question_id: r.question_id, valor: r.valor ?? '' }))
  ...
  if (form.envio_whatsapp) {
    ...
    try {
      ...
      if (modo === 'baileys') {
        await enviarViaBaileys(form.envio_whatsapp, msg, instanciaId)
      } else {
        await enviarMensagemWhatsApp(form.envio_whatsapp, msg)
      }
    } catch { /* não crítico */ }
  }
  ...
  if (form.webhook_url) {
    fetch(form.webhook_url, {
      ...
      body: JSON.stringify({ form_id: form.id, form_titulo: form.titulo, lead_id: leadId, respostas, utm_source, utm_medium, utm_campaign }),
    }).catch(() => {})
  }

  return NextResponse.json({ ok: true, lead_id: leadId, response_id: response?.id })
}
