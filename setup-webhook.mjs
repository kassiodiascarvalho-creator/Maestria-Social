...
/**
 * 🔧 Setup Completo do Webhook WhatsApp
 * 
 * 1. Verifica credenciais
 * 2. Registra o webhook
 * 3. Roda testes
 */
...
const SUPABASE_ANON_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind6dXVudXlyZ3B3am9oZmJuZ2xmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MDk4MTcwMDQsImV4cCI6MjAyNTM5MzAwNH0.YWTWkNrpqCfqMHCmUe8D1rOzrC-TqnBVsJFfH9mPGjc";
...
async function registrarWebhook() {
  ...
  try {
    ...
    if (Array.isArray(verify.data) && verify.data.length > 0) {
      console.log(`\n✅ Webhooks registrados: ${(verify.data as []).length}`);
      return true;
    } else {
